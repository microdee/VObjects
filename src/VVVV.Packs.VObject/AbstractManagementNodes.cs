...
namespace VVVV.Nodes.VObjects
{
    ...
    public abstract class ParentNode : IPluginEvaluate
    {
    ...
    public abstract class ConstructObjectNode : IPluginEvaluate
    {
        ...
        protected List<int> Removables = new List<int>();
        public int CurrObj;
    ...
    public abstract class AddObjectNode : IPluginEvaluate, IPartImportsSatisfiedNotification
    {
        [Import]
        ...
        public GenericBinSizedInput FSource;
        ...
        public void Evaluate(int SpreadMax)
        {
            if (FParent.IsConnected && FSource.Connected)
            {
                ...
                for(int i=0; i<FParent.SliceCount; i++)
                {
                    this.CurrParent = i;
                    bool added = false;
                    for (int j = 0; j < FSource[i].Count; j++ )
                    {
                        this.CurrSource = j;
